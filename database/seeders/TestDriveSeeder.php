<?php

namespace Database\Seeders;

use Illuminate\Database\Seeder;
use App\Models\TestDrive;
use App\Models\User;
use App\Models\CarVariant;
use App\Models\Showroom;

class TestDriveSeeder extends Seeder
{
    public function run(): void
    {
        $users = User::where('role','user')->get();
        $variants = CarVariant::pluck('id');
        $showrooms = Showroom::all();
        if ($variants->isEmpty() || $showrooms->isEmpty()) return;

        $count = 120;
        for ($i = 1; $i <= $count; $i++) {
            $user = optional($users->random())->id;
            $variantId = $variants->random();
            $showroom = $showrooms->random();
            $date = now()->addDays(rand(1,20));
            $time = str_pad((string) rand(9,16), 2, '0', STR_PAD_LEFT) . ':' . (rand(0,1) ? '00' : '30');
            TestDrive::create([
                'test_drive_number' => 'TD-' . date('Ymd') . '-' . str_pad((string) $i, 4, '0', STR_PAD_LEFT),
                'user_id' => $user,
                'car_variant_id' => $variantId,
                'showroom_id' => $showroom->id,
                'preferred_date' => $date->toDateString(),
                'preferred_time' => $time,
                'duration_minutes' => [20,30,45,60][array_rand([20,30,45,60])],
                'location' => $showroom->address,
                'notes' => null,
                'special_requirements' => null,
                'has_experience' => (bool) rand(0,1),
                'experience_level' => ['beginner','intermediate','advanced'][array_rand(['beginner','intermediate','advanced'])],
                'status' => ['pending','confirmed','completed','cancelled'][array_rand(['pending','confirmed','completed','cancelled'])],
                'test_drive_type' => ['individual','group'][array_rand(['individual','group'])],
                'confirmed_at' => rand(0,1) ? now() : null,
                'completed_at' => null,
                'feedback' => null,
                'satisfaction_rating' => null,
            ]);
        }
    }
}


