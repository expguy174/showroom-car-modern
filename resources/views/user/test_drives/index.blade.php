              ...
              <div class="p-4 border rounded-xl flex items-start gap-4">
                ...
                <div class="flex-1 min-w-0">
                  <div class="flex flex-wrap items-center gap-2">
                    <a href="{{ route('car_variants.show', $td->car_variant_id) }}" class="font-semibold text-gray-900 hover:text-indigo-700 truncate">#{{ $td->test_drive_number ?? $td->id }} • {{ optional(optional($td->carVariant)->carModel)->name }} {{ $td->carVariant->name ?? '' }}</a>
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  form.addEventListener('submit', async function(e){
    ...
    try {
      const res = await fetch(`{{ route('test_drives.book') }}`, {
        method: 'POST',
