        ...
        @if($wishlistItems->count() > 0)
                ...
                <form id="filter-form" class="inventory-filter-form flex items-center gap-3 p-4 bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg border border-white/50" method="GET">
                    <div class="flex-none">
                        <select name="type" class="px-3 pr-8 py-2 bg-white border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                            ...
                            <option value="car_variant" {{ ($type ?? '') === 'car_variant' ? 'selected' : '' }}>Xe hơi</option>
                            <option value="accessory" {{ ($type ?? '') === 'accessory' ? 'selected' : '' }}>Phụ kiện</option>
                        </select>
                    </div>
                    <div class="flex-1"></div>
                    <div class="relative w-56 sm:w-64 md:w-80 mx-auto">
                        <input type="text" name="q" value="{{ $q ?? '' }}" placeholder="Tìm theo tên..." class="w-full pl-10 pr-4 py-2 bg-white border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
                ...
                @empty
                    <div class="col-span-full">
                        <div class="text-center py-16 w-full max-w-md mx-auto">
                            ...
                            @php $typeLabel = ($type ?? '') === 'accessory' ? 'Phụ kiện' : ((($type ?? '') === 'car_variant') ? 'Xe hơi' : 'Sản phẩm'); @endphp
                            <div class="mt-3 font-semibold text-gray-900">Không có {{ strtolower($typeLabel) }} phù hợp</div>
                            <div class="text-sm text-gray-600">Không tìm thấy {{ strtolower($typeLabel) }} phù hợp. Hãy thử thay đổi bộ lọc hoặc từ khóa.</div>
                        </div>
                    </div>
                @endforelse
            </div>
            ...
            @if($wishlistItems->hasPages())
                <div id="wishlist-pagination" class="mb-6">
                    @include('components.pagination-modern', ['paginator' => $wishlistItems])
                </div>
            @else
