            ...
            <nav class="flex" aria-label="Breadcrumb">
                <ol class="flex items-center space-x-4">
                    ...
                    <li>
                        <div class="flex items-center">
                            <i class="fas fa-chevron-right text-gray-400 mx-2"></i>
                            <a href="{{ route('car_models.show', $variant->carModel->id ?? 1) }}" class="text-gray-500 hover:text-blue-600 transition-colors">{{ $variant->carModel->name ?? 'Model' }}</a>
                        </div>
                    </li>
                    <li>
                        <div class="flex items-center">
            ...
            <div class="space-y-6 lg:col-span-7">
                ...
                <div class="inline-block bg-gradient-to-r from-blue-50 to-indigo-50 rounded-3xl p-4 lg:p-5 border border-blue-100">
                    <div class="flex items-center gap-3">
                        ...
                        @if($variant->is_available)
                        <form action="{{ route('cart.add') }}" method="POST" class="add-to-cart-form flex items-center gap-2" data-item-type="car_variant" data-item-id="{{ $variant->id }}">
                            @csrf
                ...
                <div class="mt-3 grid grid-cols-2 gap-2">
                    ...
                    <form action="{{ route('cart.add') }}" method="POST" class="w-full add-to-cart-form" data-item-type="car_variant" data-item-id="{{ $variant->id }}">
                        @csrf
                        <input type="hidden" name="item_type" value="car_variant">
                        <input type="hidden" name="item_id" value="{{ $variant->id }}">
                        <input type="hidden" name="color_id" value="" id="selected-color-id">
