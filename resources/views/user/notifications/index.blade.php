...
<script>
	(function() {
		...
		async function load(pageToLoad) {
			...
			try {
				const res = await fetch(`{{ route('notifications.index') }}?page=${pageToLoad}`, {
					headers: {
				...
				loading = false;
				if (pageToLoad === 1 && btnRefresh && refreshHTML) {
					btnRefresh.disabled = false;
					btnRefresh.innerHTML = refreshHTML;
				}
				if (pageToLoad > 1 && btnMore && moreHTML) {
					if (hasMore) {
						btnMore.disabled = false;
						btnMore.innerHTML = moreHTML;
					} else {
						btnMore.disabled = true;
						btnMore.classList.add('hidden');
						btnMore.style.display = 'none';
		...
		btnRefresh.addEventListener('click', function() {
			...
			hasMore = true;
			noMoreEl.classList.add('hidden');
			load(1).then(() => setTimeout(checkAndShowEmptyState, 100));
		});
		...
		btnDeleteAll.addEventListener('click', async function() {
			...
			try {
				const res = await fetch(`{{ route('notifications.delete-all') }}`, {
					method: 'DELETE',
					headers: {
						'X-Requested-With': 'XMLHttpRequest',
						'X-CSRF-TOKEN': '{{ csrf_token() }}',
