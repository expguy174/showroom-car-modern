    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sm:p-8">
                <form id="sa-create-form" method="POST" action="{{ route('user.service-appointments.store') }}" novalidate>
                    @csrf
                ...
                <div>
                    ...
                    <input type="date" name="appointment_date" id="appointment_date" value="{{ old('appointment_date') }}" class="w-full rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500" required>
                ...
                <div>
                    ...
                    <input type="time" name="appointment_time" id="appointment_time" value="{{ old('appointment_time') }}" class="w-full rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500" required>
                ...
                <div>
                    ...
                    <input type="text" name="vehicle_registration" value="{{ old('vehicle_registration') }}" class="w-full rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500" placeholder="VD: 30A-123.45">
                    ...
                    <select name="service_id" class="w-full rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500" required>
                        ...
                        @foreach($services as $service)
                            <option value="{{ $service->id }}" 
                                    @selected(old('service_id', $selectedService?->id) == $service->id)
                ...
                <div>
                    ...
                    <input type="number" step="1000" name="estimated_cost" value="{{ old('estimated_cost') }}" class="w-full rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500" placeholder="0" readonly>
                    ...
                    <div class="mt-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                        <div class="flex items-start gap-3">
                            ...
                            <div class="flex-1 min-w-0">
                                <h4 class="font-medium text-blue-900">{{ $selectedService->name }}</h4>
                                <p class="text-sm text-blue-700 mt-1">{{ $selectedService->description }}</p>
                                <div class="flex items-center gap-4 mt-2 text-sm text-blue-600">
                                    <span><i class="fas fa-tag mr-1"></i>
                                        @if($selectedService->price > 0)
                                            {{ number_format($selectedService->price, 0, ',', '.') }} đ
                                        @else
                                            Miễn phí
                                        @endif
                                    </span>
                                    ...
                                    <span><i class="fas fa-clock mr-1"></i>
                                        @if($selectedService->duration_minutes < 60)
                                            {{ $selectedService->duration_minutes }} phút
                                        @elseif($selectedService->duration_minutes < 1440)
                                            {{ round($selectedService->duration_minutes / 60) }} giờ
                                        @else
                                            {{ round($selectedService->duration_minutes / 1440) }} ngày
                                        @endif
                                    </span>
                                    @endif
                                </div>
                ...
                <div class="md:col-span-2 lg:col-span-3">
                    ...
                    <textarea name="requested_services" rows="3" class="w-full rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500" placeholder="VD: Kiểm tra thêm lốp xe, thay bóng đèn...">{{ old('requested_services') }}</textarea>
