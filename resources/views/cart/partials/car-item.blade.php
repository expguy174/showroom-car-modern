...
<tr class="cart-item-row" data-id="{{ $item->id }}">
    <td class="align-top" data-label="Ảnh">
        <a href="{{ route('car_variants.show', $item->item->slug ?? $item->item->id) }}" class="block w-24 h-20 rounded-lg overflow-hidden bg-gray-100 border">
    ...
    <td class="align-top" data-label="Sản phẩm">
        <div class="min-w-0">
            <div class="font-bold text-gray-900 truncate">
                <a href="{{ route('car_variants.show', $item->item->slug ?? $item->item->id) }}" class="hover:underline">{{ $item->item->name }}</a>
            </div>
            ...
            <div class="mt-1 flex items-center gap-2">
                @foreach($item->item->colors as $color)
                    <button type="button" class="color-option w-6 h-6 rounded-full border-2 {{ $item->color_id == $color->id ? 'border-blue-500 ring-2 ring-blue-200' : 'border-gray-300' }}" title="{{ $color->color_name }}"
                        ...
                        data-update-url="{{ route('cart.update', $item->id) }}" data-csrf="{{ csrf_token() }}"></button>
                @endforeach
            </div>
            @endif
        </div>
    ...
    <td class="align-top" data-label="Số lượng">
        <div class="inline-flex items-center border border-gray-300 rounded-lg overflow-hidden">
            ...
            <input type="number" value="{{ $item->quantity }}" min="1" step="1" inputmode="numeric" pattern="[0-9]*" class="w-14 text-center border-0 cart-qty-input" data-update-url="{{ route('cart.update', $item->id) }}" data-csrf="{{ csrf_token() }}" data-id="{{ $item->id }}">
