<footer class="bg-gray-950 text-gray-300">
    <div class="container mx-auto px-4 py-12">
        ...
        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8 lg:gap-16">
            ...
            <div class="flex flex-wrap gap-16 lg:gap-16">
                <div class="flex flex-col space-y-3">
                    ...
                    <a href="{{ route('products.index') }}" class="text-gray-400 hover:text-white transition-colors text-sm">Tất cả</a>
                    <a href="{{ route('products.index', ['type' => 'car']) }}" class="text-gray-400 hover:text-white transition-colors text-sm">Xe hơi</a>
                    <a href="{{ route('products.index', ['type' => 'accessory']) }}" class="text-gray-400 hover:text-white transition-colors text-sm">Phụ kiện</a>
                </div>

                <div class="flex flex-col space-y-3">
                    <h4 class="text-white font-semibold text-sm uppercase tracking-wide">Thông tin</h4>
                    <a href="{{ route('blogs.index') }}" class="text-gray-400 hover:text-white transition-colors text-sm">Tin tức</a>
                    <a href="{{ route('user.promotions.index') }}" class="text-gray-400 hover:text-white transition-colors text-sm">Khuyến mãi</a>
                    <a href="{{ route('user.showrooms.index') }}" class="text-gray-400 hover:text-white transition-colors text-sm">Showroom</a>
                </div>

                <div class="flex flex-col space-y-3">
                    <h4 class="text-white font-semibold text-sm uppercase tracking-wide">Hỗ trợ</h4>
                    <a href="{{ route('about') }}" class="text-gray-400 hover:text-white transition-colors text-sm">Về chúng tôi</a>
                    <a href="{{ route('finance.calculator') }}" class="text-gray-400 hover:text-white transition-colors text-sm">Tính trả góp</a>
                    <a href="{{ route('contact') }}" class="text-gray-400 hover:text-white transition-colors text-sm">Liên hệ tư vấn</a>
                </div>
            </div>
        ...
        <div class="border-t border-gray-800 mt-12 pt-8 flex flex-col sm:flex-row justify-between items-center text-sm text-gray-500">
            <p>© {{ date('Y') }} AutoLux. Tất cả quyền được bảo lưu.</p>
