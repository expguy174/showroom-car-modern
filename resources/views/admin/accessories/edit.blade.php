...
<div class="bg-white rounded-xl shadow-sm border border-gray-200 max-w-4xl mx-auto">
        ...
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-xl font-semibold text-gray-900">
                    ...
                    Cập nhật phụ kiện: {{ $accessory->name }}
                </h1>
                <p class="text-sm text-gray-600 mt-1">Chỉnh sửa thông tin phụ kiện xe hơi</p>
            </div>
    ...
    {{-- Form --}}
    <form action="{{ route('admin.accessories.update', $accessory->id) }}" method="POST" enctype="multipart/form-data" class="p-6">
        @csrf
        @method('PUT')
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" name="name" id="name" 
                                   ...
                                   value="{{ old('name', $accessory->name) }}" required placeholder="Ví dụ: Lót sàn, Phim cách nhiệt...">
                            @error('name')
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="4" 
                                      ...
                                      placeholder="Mô tả về phụ kiện...">{{ old('description', $accessory->description) }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input type="number" name="price" id="price" 
                                       ...
                                       value="{{ old('price', $accessory->price) }}" required placeholder="0" min="0" step="1000">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <div class="flex items-center justify-center w-32 h-32 bg-gray-50 border border-gray-200 rounded-lg">
                                <img src="{{ $accessory->image_path }}" alt="Current Image" class="max-w-full max-h-full object-contain">
                            </div>
                        </div>
                        @endif
                        ...
                        <div>
                            <label for="image" class="block text-sm font-medium text-gray-700 mb-2">
                                {{ $accessory->image_path ? 'Thay đổi hình ảnh' : 'Tải lên hình ảnh' }}
                            </label>
                                ...
                                <div class="space-y-1 text-center">
                                    ...
                                    <div class="flex text-sm text-gray-600">
                                        <label for="image" class="relative cursor-pointer bg-white rounded-md font-medium text-blue-600 hover:text-blue-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-blue-500">
                                            <span>{{ $accessory->image_path ? 'Chọn hình ảnh mới' : 'Tải lên hình ảnh' }}</span>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="number" name="sort_order" id="sort_order" 
                                       ...
                                       value="{{ old('sort_order', $accessory->sort_order ?? 0) }}" min="0" placeholder="0">
                                @error('sort_order')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
        ...
        reader.onload = function(e) {
            ...
            preview.innerHTML = `
                <div class="flex items-center justify-center">
                    <img src="${e.target.result}" alt="New image preview" class="h-20 w-20 object-cover border border-gray-200 rounded-lg">
                </div>
            `;
        };
