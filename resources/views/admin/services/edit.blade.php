...
<div class="bg-white rounded-xl shadow-sm border border-gray-200 max-w-7xl mx-auto">
    ...
    {{-- Form --}}
    <form id="serviceForm" action="{{ route('admin.services.update', $service) }}" method="POST" class="p-6">
        @csrf
        @method('PUT')
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <select name="category" id="category" 
                                ...
                                <option value="maintenance" {{ old('category', $service->category) == 'maintenance' ? 'selected' : '' }}>Bảo dưỡng</option>
                                <option value="repair" {{ old('category', $service->category) == 'repair' ? 'selected' : '' }}>Sửa chữa</option>
                                <option value="diagnostic" {{ old('category', $service->category) == 'diagnostic' ? 'selected' : '' }}>Chẩn đoán</option>
                                <option value="cosmetic" {{ old('category', $service->category) == 'cosmetic' ? 'selected' : '' }}>Làm đẹp</option>
                                <option value="emergency" {{ old('category', $service->category) == 'emergency' ? 'selected' : '' }}>Khẩn cấp</option>
                            </select>
                            @error('category')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <textarea name="requirements" id="requirements" rows="4" 
                                      ...
                                      placeholder="Các yêu cầu trước khi thực hiện dịch vụ...">{{ old('requirements', $service->requirements) }}</textarea>
                            @error('requirements')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="notes" id="notes" rows="3" 
                                      ...
                                      placeholder="Ghi chú thêm về dịch vụ...">{{ old('notes', $service->notes) }}</textarea>
                            @error('notes')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
