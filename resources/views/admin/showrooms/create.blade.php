...
<div class="bg-white rounded-xl shadow-sm border border-gray-200 max-w-7xl mx-auto">
    ...
    <div class="px-6 py-4 border-b border-gray-200">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-gray-600 mt-1">Thêm showroom trưng bày xe mới vào hệ thống</p>
            </div>
            <a href="{{ route('admin.showrooms.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-500 hover:bg-gray-600 text-white text-sm font-medium rounded-lg transition-colors">
    ...
    {{-- Form --}}
    <form id="showroomForm" action="{{ route('admin.showrooms.store') }}" method="POST" class="p-6">
        @csrf
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="4"
                                class="block w-full px-3 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                placeholder="Nhập mô tả về showroom...">{{ old('description') }}</textarea>
                        </div>
                    </div>
                </div>
        ...
        <div class="flex items-center justify-between pt-6 mt-6 border-t border-gray-200">
            <a href="{{ route('admin.showrooms.index') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-lg text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
...
<script>
    ...
    document.getElementById('showroomForm').addEventListener('submit', async function(e) {
        ...
        try {
            ...
            if (response.ok) {
                if (window.showMessage) {
                    window.showMessage(data.message || 'Đã thêm showroom thành công!', 'success');
                }

                // Redirect
                setTimeout(() => {
                    window.location.href = '{{ route("admin.showrooms.index") }}';
                }, 1500);
            } else {
            ...
            if (window.showMessage) {
                window.showMessage(error.message || 'Có lỗi xảy ra khi thêm showroom!', 'error');
            }
        } finally {
