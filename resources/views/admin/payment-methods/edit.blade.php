...
<div class="bg-white rounded-xl shadow-sm border border-gray-200 max-w-7xl mx-auto">
    ...
    <div class="px-6 py-4 border-b border-gray-200">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-gray-600 mt-1">Cập nhật thông tin phương thức thanh toán</p>
            </div>
    ...
    {{-- Form --}}
    <form id="paymentMethodForm" action="{{ route('admin.payment-methods.update', $paymentMethod) }}" method="POST" class="p-6" novalidate>
        @csrf
        @method('PUT')
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" name="name" id="name"
                                   ...
                                   value="{{ old('name', $paymentMethod->name) }}" placeholder="VD: Thẻ tín dụng Visa">
                            @error('name')
                        ...
                        <div>
                            ...
                            <input type="text" name="code" id="code"
                                   ...
                                   value="{{ old('code', $paymentMethod->code) }}" placeholder="VD: cash, vnpay, momo">
                            @error('code')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="provider" id="provider"
                                   ...
                                   value="{{ old('provider', $paymentMethod->provider) }}" placeholder="VD: Visa, MasterCard, VNPay">
                            @error('provider')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <select name="type" id="type" 
                                ...
                                <option value="online" {{ old('type', $paymentMethod->type) == 'online' ? 'selected' : '' }}>Trực tuyến</option>
                                <option value="offline" {{ old('type', $paymentMethod->type) == 'offline' ? 'selected' : '' }}>Trực tiếp</option>
                            </select>
                            @error('type')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <textarea name="notes" id="notes" rows="3" 
                                      ...
                                      placeholder="Ghi chú về phương thức thanh toán...">{{ old('notes', $paymentMethod->notes) }}</textarea>
                            @error('notes')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="number" name="fee_flat" id="fee_flat" step="1000" min="0"
                                   ...
                                   value="{{ old('fee_flat', $paymentMethod->fee_flat ? number_format($paymentMethod->fee_flat, 0, ',', '') : 0) }}" placeholder="0">
                        ...
                        <div>
                            ...
                            <input type="number" name="fee_percent" id="fee_percent" step="0.01" min="0" max="100"
                                   ...
                                   value="{{ old('fee_percent', $paymentMethod->fee_percent == 0 ? 0 : $paymentMethod->fee_percent) }}" placeholder="0">
                            ...
                            <div id="config-container" class="space-y-3">
                                @php
                                    ...
                                    if ($paymentMethod->config) {
                                        $existingConfig = is_string($paymentMethod->config) 
                                ...
                                @if(count($existingConfig) > 0)
                                    ...
                                    <div class="config-row flex items-center gap-2">
                                    ...
                                    @endforeach
                                @endif
                                {{-- Nếu không có config, container sẽ trống --}}
                            </div>
                            <button type="button" id="add-config-row" class="mt-4 inline-flex items-center px-3 py-1 border border-gray-300 rounded-md text-sm text-gray-700 bg-white hover:bg-gray-50">
                        ...
                        <div>
                            ...
                            <input type="number" name="sort_order" id="sort_order" 
                                   ...
                                   value="{{ old('sort_order', $paymentMethod->sort_order) }}" placeholder="0">
                            @error('sort_order')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
...
document.getElementById('paymentMethodForm').addEventListener('submit', function(e) {
    ...
    .then(data => {
        if (data.success) {
            ...
            if (window.showMessage) {
                window.showMessage(data.message || 'Cập nhật phương thức thanh toán thành công!', 'success');
            }
    ...
    .catch(error => {
        ...
        if (error.status === 422 && error.data && error.data.errors) {
            ...
            if (window.showMessage) {
                window.showMessage(error.message || 'Có lỗi xảy ra khi cập nhật phương thức thanh toán', 'error');
            }
        }
    })
    .finally(() => {
...
function validatePaymentMethodForm() {
    ...
    if (!feeFlatField) {
        console.error('Fee flat field not found');
        return { isValid: true };
    }
    ...
    if (!feePercentField) {
        console.error('Fee percent field not found');
        return { isValid: true };
    }
    ...
    if (!sortOrderField) {
        console.error('Sort order field not found');
        return { isValid: true };
    }
