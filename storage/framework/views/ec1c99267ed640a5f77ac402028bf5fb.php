                            ...
                            <div class="flex items-start sm:items-center justify-between gap-2">
                                <div class="min-w-0">
                                    <div class="flex items-center gap-2 flex-wrap">
                                        <a href="<?php echo e(route('user.service-appointments.show', $appointment->id)); ?>" class="text-gray-800 font-semibold truncate hover:underline">#<?php echo e($appointment->appointment_number); ?></a>
                                        <span class="hidden xs:inline text-gray-400">•</span>
                                        <span class="text-gray-500 text-xs sm:text-sm"><?php echo e(optional($appointment->appointment_date)->format('d/m/Y')); ?> <?php echo e($appointment->appointment_time); ?></span>
                                    </div>
                                    <div class="text-xs sm:text-sm text-gray-500 mt-1 truncate">
                                        <?php echo e($appointment->carVariant->carModel->carBrand->name); ?> <?php echo e($appointment->carVariant->carModel->name); ?> • 
                                        <?php echo e($appointment->service ? $appointment->service->name : 'Chưa chọn dịch vụ'); ?>

                                    </div>
                                </div>
                                <div class="flex items-center gap-2 shrink-0">
                                    <span class="px-2 py-0.5 rounded-full text-xs whitespace-nowrap inline-flex items-center <?php echo e(\App\Helpers\ServiceAppointmentHelper::statusBadgeClass($appointment->status)); ?>" data-role="status-badge"><?php echo e(\App\Helpers\ServiceAppointmentHelper::statusLabel($appointment->status)); ?></span>
                                </div>
                            </div>
                        </div>
                        <div class="flex items-center justify-between">
                            <div class="flex flex-wrap items-center gap-2 sm:gap-3 text-[11px] sm:text-xs text-gray-500">
                                <span>Showroom: <span class="font-medium text-gray-700"><?php echo e($appointment->showroom->name); ?></span></span>
                            </div>
                            <div class="flex items-center gap-2">
                                <a href="<?php echo e(route('user.service-appointments.show', $appointment->id)); ?>" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-gray-200 text-gray-700 hover:bg-gray-50 text-xs"><i class="fas fa-eye"></i> Chi tiết</a>
                                <button type="button" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold bg-rose-600 text-white hover:bg-rose-700 disabled:bg-gray-100 disabled:text-gray-400 disabled:cursor-not-allowed js-sa-cancel-one"
                                  data-cancel-url="<?php echo e(route('user.service-appointments.cancel', $appointment->id)); ?>" <?php echo e($appointment->status === 'scheduled' ? '' : 'disabled'); ?>>
...
<script>
(function(){
  const cancelledBadgeClass = "<?php echo e(\App\Helpers\ServiceAppointmentHelper::statusBadgeClass('cancelled')); ?>";
  document.addEventListener('click', async function(e){
    ...
    try{
      ...
      const res = await fetch(url, { method:'PUT', headers:{ 'X-Requested-With':'XMLHttpRequest','X-CSRF-TOKEN':'<?php echo e(csrf_token()); ?>','Accept':'application/json' } });
...
<?php /**PATH C:\Users\forev\showroom-car-modern\resources\views/user/service-appointments/partials/list.blade.php ENDPATH**/ ?>