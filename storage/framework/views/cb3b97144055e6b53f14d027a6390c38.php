<?php $__env->startSection('title', 'Sản phẩm - Showroom hiện đại'); ?>
        ...
        <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-10 relative">
            ...
            <div class="mt-6 grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div class="bg-white/10 rounded-xl p-4 text-white">
                    ...
                    <div class="text-2xl font-bold"><?php echo e(number_format(($stats['total_variants'] ?? 0) + ($stats['acc_total'] ?? 0))); ?></div>
                </div>
                <div class="bg-white/10 rounded-xl p-4 text-white">
                    <div class="text-xs uppercase opacity-80">Mẫu xe</div>
                    <div class="text-2xl font-bold"><?php echo e(number_format($stats['total_variants'] ?? 0)); ?></div>
                </div>
                <div class="bg-white/10 rounded-xl p-4 text-white">
                    <div class="text-xs uppercase opacity-80">Phụ kiện</div>
                    <div class="text-2xl font-bold"><?php echo e(number_format($stats['acc_total'] ?? 0)); ?></div>
                </div>
                <div class="bg-white/10 rounded-xl p-4 text-white">
                    <div class="text-xs uppercase opacity-80">Đang có sẵn</div>
                    <div class="text-2xl font-bold"><?php echo e(number_format(($stats['in_stock'] ?? 0) + ($stats['acc_in_stock'] ?? 0))); ?></div>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="flex flex-col lg:flex-row gap-8">
            ...
            <aside class="lg:w-72 w-full">
                ...
                <details class="lg:hidden bg-white rounded-xl shadow-sm">
                    ...
                    <div class="px-4 pb-4">
                        <?php echo $__env->make('user.products.partials.filters', ['formId' => 'filter-form-mobile'], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
                        </div>
                </details>
                ...
                <div class="hidden lg:block bg-white rounded-xl shadow-sm p-5 sticky top-6">
                    ...
                    <?php echo $__env->make('user.products.partials.filters', ['formId' => 'filter-form'], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
                            </div>
            </aside>
            ...
            <section class="flex-1 min-w-0">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
                    <div id="inv-count" class="text-sm text-gray-600">
                        <?php if(isset($mode) && $mode === 'accessory' && isset($accessories)): ?>
                            <?php echo e(number_format($accessories->total())); ?> kết quả (Phụ kiện)
                        <?php elseif(isset($mode) && $mode === 'variants' && isset($variants)): ?>
                            <?php echo e(number_format($variants->total())); ?> kết quả (Xe hơi)
                        <?php else: ?>
                            <?php echo e(number_format(($stats['total_variants'] ?? 0) + ($stats['acc_total'] ?? 0))); ?> kết quả (Tất cả)
                        <?php endif; ?>
                        </div>
                    <form id="sort-form" method="GET" action="<?php echo e(route($routeName ?? 'products.index')); ?>" class="inventory-filter-form flex items-center gap-2">
                        <?php $__currentLoopData = request()->except(['sort','order','q','page','cars_page','acc_page']); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $k => $v): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <input type="hidden" name="<?php echo e($k); ?>" value="<?php echo e($v); ?>">
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        <div class="flex items-center gap-2 flex-1 min-w-[220px] sm:min-w-[260px] max-w-xl">
                            <div class="relative flex-1">
                                <input type="search" name="q" value="<?php echo e(request('q')); ?>" placeholder="Tìm kiếm" aria-label="Tìm kiếm"
                        ...
                        <select name="sort" class="text-sm border-gray-200 rounded-lg">
                            ...
                            <?php if($t === 'accessory'): ?>
                                <option value="name" <?php echo e(request('sort','name')==='name' ? 'selected' : ''); ?>>Tên phụ kiện</option>
                                <option value="price" <?php echo e(request('sort')==='price' ? 'selected' : ''); ?>>Giá</option>
                            <?php elseif($t === 'car'): ?>
                                <option value="name" <?php echo e(request('sort','name')==='name' ? 'selected' : ''); ?>>Tên xe</option>
                                <option value="price" <?php echo e(request('sort')==='price' ? 'selected' : ''); ?>>Giá</option>
                            <?php else: ?>
                                <option value="name" <?php echo e(request('sort','name')==='name' ? 'selected' : ''); ?>>Tên</option>
                                <option value="price" <?php echo e(request('sort')==='price' ? 'selected' : ''); ?>>Giá</option>
                            <?php endif; ?>
                        </select>
                        <select name="order" class="text-sm border-gray-200 rounded-lg">
                            <option value="asc" <?php echo e(request('order','asc')==='asc' ? 'selected' : ''); ?>>Tăng dần</option>
                            <option value="desc" <?php echo e(request('order')==='desc' ? 'selected' : ''); ?>>Giảm dần</option>
                            </select>
                    </form>
            </div>

                <div id="inv-results">
                <?php if(isset($mode) && $mode === 'accessory'): ?>
                    <?php if($accessories->count()): ?>
                        <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
                            <?php $__currentLoopData = $accessories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $accessory): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <?php echo $__env->make('components.accessory-card', ['accessory' => $accessory], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </div>
                        <div class="mt-8">
                            <?php echo $__env->make('components.pagination-modern', ['paginator' => $accessories->withQueryString()], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
                        </div>
                    <?php else: ?>
                    ...
                    <?php endif; ?>
                <?php elseif(isset($mode) && $mode === 'variants'): ?>
                    <?php if($variants->count()): ?>
                        ...
                        <div class="mt-8">
                            <?php echo $__env->make('components.pagination-modern', ['paginator' => $variants->withQueryString()], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
                        </div>
                    <?php else: ?>
                        <div class="bg-white rounded-xl p-10 text-center text-gray-600 shadow-sm">
                            <i class="fas fa-search text-3xl text-gray-300"></i>
        ...
        <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-16 relative z-10">
            <div class="text-center text-white">
                ...
                <div class="flex flex-col sm:flex-row gap-4 justify-center items-center">
                    ...
                    <a href="<?php echo e(route('test-drives.book')); ?>" 
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\forev\showroom-car-modern\resources\views/user/products/index.blade.php ENDPATH**/ ?>