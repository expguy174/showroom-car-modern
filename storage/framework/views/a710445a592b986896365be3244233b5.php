<?php $__env->startSection('title', 'Đặt lịch lái thử'); ?>
...
<div class="container mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-6 sm:py-8">
	<div class="flex items-start sm:items-center justify-between gap-3 mb-4 sm:mb-6">
		...
		<a href="<?php echo e(route('test-drives.index')); ?>" class="inline-flex items-center gap-2 px-3 py-2 rounded-lg border border-gray-200 text-gray-700 hover:bg-gray-50 text-sm font-semibold shrink-0"><i class="fas fa-arrow-left"></i> Quay lại</a>
	</div>
	...
	<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
		...
		<div class="lg:col-span-2">
			<div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sm:p-8">
				<form method="POST" action="<?php echo e(route('test-drives.book')); ?>" id="testdrive-create-form" novalidate>
				<?php echo csrf_field(); ?>
				...
				<div>
					...
					<select name="car_variant_id" class="w-full border rounded-xl px-3 py-2 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" required>
						...
						<?php $__currentLoopData = ($variants ?? collect()); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $v): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
							<option value="<?php echo e($v->id); ?>"><?php echo e(trim(((optional(optional($v->carModel)->carBrand)->name) ? optional(optional($v->carModel)->carBrand)->name.' • ' : '') . ((optional($v->carModel)->name) ? optional($v->carModel)->name.' • ' : '') . ($v->name ?? ''))); ?></option>
				...
				<div>
					...
					<select name="showroom_id" id="showroom_id" class="w-full border rounded-xl px-3 py-2">
						...
						<?php $__currentLoopData = ($showrooms ?? collect()); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $s): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
		...
		try {
			...
			if (res.ok && data && data.success){
				...
				const go = () => {
					...
					else { window.location.href = "<?php echo e(route('test-drives.index')); ?>"; }
				};
				setTimeout(go, 1400);
			} else {
...
(function(){
  ...
  btn.addEventListener('click', async function(){
    ...
    try {
      const res = await fetch("<?php echo e(route('test-drives.check-availability')); ?>", { 
        method:'POST', 
        headers:{ 
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\forev\showroom-car-modern\resources\views/user/test-drives/create.blade.php ENDPATH**/ ?>