...
<head>
    ...
    <meta name="profile-status" content="<?php echo e(session('status')); ?>">
    <meta property="og:title" content="<?php echo $__env->yieldContent('title', 'AutoLux Showroom'); ?>">
    ...
    <style>
        ...
        #car-variant-page .action-btn:hover i {
            transform: scale(1.1);
        }

        #car-variant-page .action-primary {
            color: #fff;
            background-image: linear-gradient(90deg, #4f46e5 0%, #4338ca 100%);
        }

        #car-variant-page .action-primary:hover {
            background-image: linear-gradient(90deg, #4338ca 0%, #3730a3 100%);
        }

        #car-variant-page .action-primary:focus-visible {
        ...
        #car-variant-page .action-ghost:hover {
            color: #3730a3;
            border-color: #c7d2fe;
        }

        #car-variant-page .action-ghost:focus-visible {
            outline: 2px solid #a5b4fc;
            outline-offset: 2px;
        }
        ...
        .accessory-show-page .action-btn i {
            font-size: 1.125rem;
            transition: transform .25s ease;
        }
        ...
        .accessory-show-page .action-btn:hover i {
            transform: scale(1.1);
        }
        ...
        .accessory-show-page .action-primary:hover {
            background-image: linear-gradient(90deg, #4338ca 0%, #3730a3 100%);
        }

        .accessory-show-page .action-primary:focus-visible {
            outline: 2px solid #a5b4fc;
            outline-offset: 2px;
        }
        ...
        .accessory-show-page .action-ghost:hover {
            color: #3730a3;
            border-color: #c7d2fe;
        }

        .accessory-show-page .action-ghost:focus-visible {
            outline: 2px solid #a5b4fc;
            outline-offset: 2px;
        }
    ...
    <?php try { $appCss = Vite::asset('resources/css/app.css'); } catch (\Throwable $e) { $appCss = null; } ?>
    <?php if($appCss): ?>
    <link rel="preload" href="<?php echo e($appCss); ?>" as="style" />
    <?php endif; ?>
    
    <?php $hasCritical=false; try { Vite::asset('resources/css/critical.css'); $hasCritical=true; } catch (\Throwable $e) { $hasCritical=false; } ?>
    <?php if($hasCritical): ?>
...
<body class="bg-white text-gray-900">
    ...
    <script>
        (function() {
            async function fetchNotifs() {
                ...
                if (!list) return;
                try {
                    var notifIndexUrl = "<?php echo e(route('notifications.index')); ?>";
            ...
            async function markAll() {
                try {
                    var notifReadAllUrl = "<?php echo e(route('notifications.read-all')); ?>";
    ...
    <script>
        ...
        document.addEventListener('click', async function(e) {
            ...
            if (!variantId) return;
            try {
                const res = await fetch(`/api/v1/inventory/by-variant?variant_id=${encodeURIComponent(variantId)}`, {
                    headers: {
                        'X-Requested-With': 'XMLHttpRequest'
                    }
                });
                ...
                if (!(data && data.success)) {
                    showMessage('Không thể tải xe trong kho', 'error');
                    return;
                }
                const units = Array.isArray(data.data) ? data.data : [];
                if (units.length === 0) {
                    showMessage('Hết hàng. Bạn có thể Đặt trước hoặc Nhận tư vấn.', 'info');
                    return;
                }
        ...
        document.addEventListener('click', async function(e) {
            ...
            try {
                ...
                if (form) {
                    ...
                    invHidden.name = 'inventory_id';
                    invHidden.value = unitId;
                ...
                showMessage('Không thể mở đặt cọc', 'error');
            }
        });
            ...
            function openCompareModal() {
                ...
                if (diffToggle) {
                    try {
                        diffToggle.checked = JSON.parse(localStorage.getItem('compare.showDiffOnly') || 'false');
                    } catch {
            ...
            function renderCompareContent() {
                ...
                const diffOnly = (() => {
                    try {
                        return JSON.parse(localStorage.getItem('compare.showDiffOnly') || 'false');
                    } catch {
                        return false;
                    }
                })();
                ...
                const hideClear = (hide) => {
                    if (clearBtn) {
                        ...
                        clearBtn.classList.toggle('sm:hidden', hide);
                        clearBtn.setAttribute('aria-hidden', hide ? 'true' : 'false');
                    }
            ...
            document.addEventListener('change', function(e) {
                ...
                if (!toggle) return;
                try {
                    localStorage.setItem('compare.showDiffOnly', JSON.stringify(!!toggle.checked));
                } catch {}
                renderCompareContent();
            });
            document.addEventListener('change', function(e) {
    ...
    <script>
        ...
        (function() {
            if (window.__bindAddToCartVanilla) return;
            window.__bindAddToCartVanilla = true;
        ...
        function refreshCartCount() {
            fetch('<?php echo e(route("user.cart.count")); ?>', {
                    headers: {
                        'X-Requested-With': 'XMLHttpRequest'
                    }
                })
        ...
        function fetchSearchSuggestions(query, suggestionsList, container) {
            ...
            const suggestions = [{
                    text: `${query} - Tất cả xe`,
                    url: `/search?q=${encodeURIComponent(query)}&category=cars`
                },
                {
                    text: `${query} - Phụ kiện`,
                    url: `/search?q=${encodeURIComponent(query)}&category=accessories`
                },
                {
                    text: `${query} - Theo hãng`,
                    url: `/search?q=${encodeURIComponent(query)}&category=cars`
                },
                {
                    text: `${query} - Theo model`,
                    url: `/search?q=${encodeURIComponent(query)}&category=cars`
                },
            ];
...
</html><?php /**PATH C:\Users\forev\showroom-car-modern\resources\views/layouts/app.blade.php ENDPATH**/ ?>