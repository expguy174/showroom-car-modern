<?php $__env->startSection('title', 'Chỉnh sửa dịch vụ: ' . $service->name); ?>
...
<div class="bg-white rounded-xl shadow-sm border border-gray-200 max-w-7xl mx-auto">
        ...
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-xl font-semibold text-gray-900">
                    ...
                    Chỉnh sửa dịch vụ: <?php echo e($service->name); ?>

                </h1>
                <p class="text-sm text-gray-600 mt-1">Cập nhật thông tin dịch vụ</p>
            </div>
    ...
    <form id="serviceForm" action="<?php echo e(route('admin.services.update', $service)); ?>" method="POST" class="p-6">
        <?php echo csrf_field(); ?>
...
unset($__errorArgs, $__bag); ?>" 
                                   value="<?php echo e(old('code', $service->code)); ?>" 
...
unset($__errorArgs, $__bag); ?>">
                                ...
                                <option value="maintenance" <?php echo e(old('category', $service->category) == 'maintenance' ? 'selected' : ''); ?>>Bảo dưỡng</option>
                                <option value="repair" <?php echo e(old('category', $service->category) == 'repair' ? 'selected' : ''); ?>>Sửa chữa</option>
                                <option value="diagnostic" <?php echo e(old('category', $service->category) == 'diagnostic' ? 'selected' : ''); ?>>Chẩn đoán</option>
                                <option value="cosmetic" <?php echo e(old('category', $service->category) == 'cosmetic' ? 'selected' : ''); ?>>Làm đẹp</option>
                                <option value="emergency" <?php echo e(old('category', $service->category) == 'emergency' ? 'selected' : ''); ?>>Khẩn cấp</option>
                            </select>
...
unset($__errorArgs, $__bag); ?>" 
                                      placeholder="Mô tả chi tiết về dịch vụ..."><?php echo e(old('description', $service->description)); ?></textarea>
...
unset($__errorArgs, $__bag); ?>" 
                                      placeholder="Các yêu cầu trước khi thực hiện dịch vụ..."><?php echo e(old('requirements', $service->requirements)); ?></textarea>
...
unset($__errorArgs, $__bag); ?>" 
                                      placeholder="Ghi chú thêm về dịch vụ..."><?php echo e(old('notes', $service->notes)); ?></textarea>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="flex items-center">
                            <input type="checkbox" name="is_featured" id="is_featured" value="1"
                                   class="h-4 w-4 text-blue-600 focus:ring-yellow-500 border-gray-300 rounded"
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    .then(data => {
        if (data.success) {
            ...
            if (window.showMessage) {
                window.showMessage(data.message || 'Cập nhật dịch vụ thành công!', 'success');
            }
    ...
    .catch(error => {
        ...
        if (error.status === 422 && error.data && error.data.errors) {
            ...
            if (window.showMessage) {
                window.showMessage(error.message || 'Có lỗi xảy ra khi cập nhật dịch vụ', 'error');
            }
        }
    })
    .finally(() => {
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\forev\showroom-car-modern\resources\views/admin/services/edit.blade.php ENDPATH**/ ?>