...
<tr class="cart-item-desktop desktop-only" data-id="<?php echo e($item->id); ?>" data-item-type="<?php echo e($item->item_type); ?>" data-color-id="<?php echo e($item->color_id); ?>" data-base-unit="<?php echo e((int) $baseUnit); ?>" data-original-price="<?php echo e((int) $originalPriceBeforeDiscount); ?>" data-current-price="<?php echo e((int) $currentPrice); ?>">
    <td class="align-middle text-center" data-label="Ảnh">
        <a href="<?php echo e(route('accessories.show', $item->item->slug ?? $item->item->id)); ?>" class="inline-block w-24 h-20 rounded-lg overflow-hidden bg-gray-100 border mx-auto">
    ...
    <td class="align-top" data-label="Sản phẩm">
        <div class="min-w-0">
            <div class="font-bold text-gray-900 truncate">
                <a href="<?php echo e(route('accessories.show', $item->item->slug ?? $item->item->id)); ?>" class="hover:underline"><?php echo e($item->item->name); ?></a>
            </div>
            <?php if(!empty($item->item->brand)): ?>
                <div class="text-sm text-gray-600 truncate"><?php echo e($item->item->brand); ?></div>
            <?php endif; ?>
            <?php if(!empty($item->item->short_description)): ?>
                <div class="text-xs text-gray-500 mt-1 line-clamp-2"><?php echo e(Str::limit(strip_tags($item->item->short_description), 90)); ?></div>
            <?php endif; ?>
            <div class="mt-2">
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.stock-badge','data' => ['stock' => $stockQuantity,'type' => 'accessory','size' => 'sm']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['stock' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($stockQuantity),'type' => 'accessory','size' => 'sm']); ?>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 mx-4 mb-4 overflow-hidden">
            ...
            <div class="p-4 pb-3">
                <div class="flex items-center gap-3">
                    <!-- Product Image -->
                    <a href="<?php echo e(route('accessories.show', $item->item->slug ?? $item->item->id)); ?>" class="flex-shrink-0 w-20 h-16 rounded-xl overflow-hidden bg-gray-100 border flex items-center justify-center">
                        <img src="<?php echo e($imageUrl); ?>" class="w-full h-full object-cover" alt="<?php echo e($item->item->name); ?>" loading="lazy" decoding="async" onerror="this.onerror=null;this.src='https://via.placeholder.com/800x600?text=No+Image';">
                    </a>
                    ...
                    <div class="min-w-0 flex-1">
                        <div class="font-bold text-gray-900 text-base leading-tight">
                            <a href="<?php echo e(route('accessories.show', $item->item->slug ?? $item->item->id)); ?>" class="hover:text-blue-600 transition-colors"><?php echo e($item->item->name); ?></a>
                        </div>
                        <?php if(!empty($item->item->brand)): ?>
                            <div class="text-sm text-gray-600 mt-1"><?php echo e($item->item->brand); ?></div>
                        <?php endif; ?>
                        <?php if(!empty($item->item->short_description)): ?>
                            <div class="text-xs text-gray-500 mt-1 line-clamp-2"><?php echo e(Str::limit(strip_tags($item->item->short_description), 80)); ?></div>
                        <?php endif; ?>
                        <div class="mt-2">
                            <?php if (isset($component)) { $__componentOriginal8c4d3e6a7bdeb5f6eaebbbf1808b98c4 = $component; } ?>
<?php if (isset($attributes)) { $__attributesOriginal8c4d3e6a7bdeb5f6eaebbbf1808b98c4 = $attributes; } ?>
...
<?php endif; ?>
                    ...
                    <!-- Delete Button -->
                    <button type="button" aria-label="Xóa" title="Xóa" class="remove-item-btn flex-shrink-0 w-8 h-8 rounded-full bg-red-50 text-red-500 flex items-center justify-center" data-id="<?php echo e($item->id); ?>" data-url="<?php echo e(route('user.cart.remove', $item->id)); ?>" data-csrf="<?php echo e(csrf_token()); ?>">
                        <i class="fas fa-trash text-sm"></i>
                    </button>
                </div>
            </div>
                ...
                <div class="flex items-center justify-between">
                    ...
                    <div class="text-right min-w-0 flex-1">
                        <div class="space-y-1">
                            ...
                            <div class="text-lg font-bold text-blue-600 mt-2">Giá: <span class="item-total" data-id="<?php echo e($item->id); ?>"><?php echo e(number_format($itemTotal,0,',','.')); ?></span> đ</div>
                        </div>
                    </div>
                </div>
            </div>
...
<?php /**PATH C:\Users\forev\showroom-car-modern\resources\views/user/cart/partials/accessory-item.blade.php ENDPATH**/ ?>