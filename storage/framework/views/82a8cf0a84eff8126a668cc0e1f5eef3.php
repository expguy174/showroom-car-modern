<?php $__env->startSection('title', 'Lịch lái thử của tôi'); ?>
...
<div class="container mx-auto px-3 sm:px-4 md:px-6 lg:px-8 pt-6 sm:pt-8 pb-6">
	<div class="flex items-center justify-between mb-4 sm:mb-6">
		...
		<div class="hidden sm:flex items-center gap-2">
			<a href="<?php echo e(route('test-drives.create')); ?>" class="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-indigo-600 text-white hover:bg-indigo-700 text-sm font-semibold">
				<i class="fas fa-plus"></i> Đặt lịch mới
			</a>
		</div>
	</div>

	<form action="<?php echo e(route('test-drives.index')); ?>" method="get" class="mb-4 sm:mb-6">
		<div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
			<div class="grid grid-cols-1 md:grid-cols-3 gap-3">
				<div class="md:col-span-2">
					<label class="block text-sm font-medium text-gray-700 mb-1">Tìm kiếm</label>
					<div class="relative">
						<input type="text" name="q" value="<?php echo e($q ?? request('q')); ?>" placeholder="Mã lịch, tên mẫu xe..." class="w-full rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 pr-10">
				...
				<div>
					...
					<select name="status" class="w-full rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500">
						...
						<?php $__currentLoopData = \App\Models\TestDrive::STATUSES; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $st): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
							<option value="<?php echo e($st); ?>" <?php if(($status ?? request('status')) === $st): echo 'selected'; endif; ?>><?php echo e((new \App\Models\TestDrive(['status'=>$st]))->status_text); ?></option>
	...
	<div id="summary-host">
		<?php echo $__env->make('user.test-drives.partials.summary', ['paginator' => $testDrives->withQueryString()], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
	</div>
	...
	<div id="testdrives-list-wrapper">
		...
		<?php else: ?>
			<?php echo $__env->make('user.test-drives.partials.list', ['testDrives' => $testDrives->withQueryString()], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
		<?php endif; ?>
	</div>
</div>
...
async function refreshList(){
	const form = document.querySelector(`form[action='<?php echo e(route('test-drives.index')); ?>']`);
...
document.addEventListener('click', async function(e){
	...
	if (cancelBtn){
		...
		showConfirmDialog('Hủy lịch lái thử?', 'Bạn có chắc chắn muốn hủy lịch này? Hành động không thể hoàn tác.', 'Hủy lịch', 'Hủy bỏ', async ()=>{
			...
			try{
				const url = `<?php echo e(url('/test-drives')); ?>/${id}/cancel`;
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\forev\showroom-car-modern\resources\views/user/test-drives/index.blade.php ENDPATH**/ ?>